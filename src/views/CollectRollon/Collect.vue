                ...
                <div class="container-fluid">
                    ...
                    <div class="row">
                        <div class="col-xl-12">
                            <div class="card-box">
                                <p class="sub-header">
                                    {{ $t('str.msg.register.collects.rollon.label') }}
                                </p>
                                <div class="form-row">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-4">
                                        <label class="col-form-label">
                                            <strong>{{ $t('string.msg.dashboard.orders.select.hub.collect.label') }}</strong>
                                        </label>

                                        <multiselect
                                            v-model="filters.collectedHub"
                                            :options="hubs"
                                            @input="filter"
                                            :close-on-select="true"
                                            track-by="_id"
                                            :custom-label="customLabelCollectedHub"
                                            :searchable="true"
                                            placeholder="Selecione"
                                        >
                                        </multiselect>
                                    ...
                                    <div class="form-group col-md-4">
                                        <label class="col-form-label">
                                            <strong>{{ $t('string.msg.dashboard.orders.select.hub.deposit.label') }}</strong>
                                        </label>
                                        <multiselect
                                            v-model="filters.depositedHub"
                                            :options="hubs"
                                            :close-on-select="true"
                                            track-by="_id"
                                            :custom-label="customLabelDepositedHub"
                                            :searchable="true"
                                            placeholder="Selecione"
                                        >
                                        </multiselect>
                                    </div>
                                    <div v-if="isAdminMaster()" class="form-group col-md-4">
                                        <label for="inputAddress" class="col-form-label">
                                ...
                                <div class="table-responsive mb-4">
                                    <table class="table table-centered table-hover mb-0" id="datatable">
                                        <thead class="table-dark">
                                            <tr>
                                                <th class="border-top-0" style="width: 8%">{{ $t('string.msg.dashboard.orders.table.init.collect.title') }}</th>
                                                <th class="border-top-0" style="width: 8%">{{ $t('string.msg.dashboard.orders.table.final.collect.title') }}</th>
                                                <th class="border-top-0" style="width: 8%">{{ $t('string.msg.dashboard.orders.table.hub.collect.title') }}</th>
                                                <th class="border-top-0" style="width: 8%">{{ $t('string.msg.dashboard.orders.table.hub.deposit.title') }}</th>
                                                <th class="border-top-0" style="width: 10%">{{ $t('string.msg.dashboard.orders.table.product.title') }}</th>
                                                <th class="border-top-0" style="width: 4%">{{ $t('string.msg.dashboard.orders.table.weight.title') }}</th>
                                                <th class="border-top-0" style="width: 4%">{{ $t('string.msg.dashboard.orders.table.quantity.title') }}</th>
                                                <th class="border-top-0" style="width: 10%">{{ $t('string.msg.dashboard.orders.table.employee.title') }}</th>
                                                <th class="border-top-0" style="width: 20%">{{ $t('string.msg.dashboard.orders.table.point.hub.title') }}</th>
                                                <th class="border-top-0" style="width: 10%">{{ $t('string.msg.dashboard.orders.table.geo.collect.title') }}<i class="fas fa-eye"></i></th>
                                                <th class="border-top-0" style="width: 5%">{{ $t('string.msg.dashboard.orders.table.status.collect.title') }}</th>
                                                <th class="border-top-0"></th>
                                            </tr>
                                        </thead>
                                        <tbody v-bind:key="item._id" v-for="(item, index) in pageOfItems" class="cursor-pointer">
                                            <tr v-if="item">
                                                <td>
                                                ...
                                                <td>
                                                    <span v-if="item.collectedHub && item.collectedHub.name">
                                                        {{ item.collectedHub.name }}
                                                    </span>
                                                </td>
                                                <td>
                                                    <span v-if="item.depositedHub && item.depositedHub.name">
                                                        {{ item.depositedHub.name }}
                                                    </span>
                                                    <span v-else-if="!item.depositedHub && item.type && item.type == 'CLOSED'">
                                                        <strong style="font-size: 30px" v-tooltip="$t('str.collect.conclude.but.no.deposited.hub')">{{ ' --' }}</strong>
                                                    </span>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    <span v-if="item.latlng && item.latlng.latitude && item.latlng.longitude">
                                                ...
                                                <td>
                                                    ...
                                                    <span v-else-if="item.type == 'OPENED'">
                                                        <span style="color: red" v-tooltip="$t('msg.this.collection.was.already.heavy')">{{ $t('string.msg.dashboard.orders.table.status.open') }}</span>
                                                    </span>
                                                </td>
                                                <td>
                                                    <button
